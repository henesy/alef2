...
codmop(Node *o, Node *l, Node *r, Node *dst)
{
	...
	case OSUB:
		if(r && r->type == OCONST && r->type != TFLOAT && r->ival == 0) {
			r = ZeroN;
			op = ANEG;
			break;
		}
...
codcond(int comp, Node *src1, Node *src2)
{
	...
	if(R0ISZERO && src1->type == OCONST)
	if(src1->ival == 0)
	if(src1->t->type != TFLOAT)
		src1 = regn(0);

	if(R0ISZERO && src2->type == OCONST)
	if(src2->ival == 0)
	...
	if(t == TFLOAT)
		instruction(AFCMPU,src1, ZeroN, src2);
	else if(uns)
		instruction(ACMPU, src1, ZeroN, src2);
	else
